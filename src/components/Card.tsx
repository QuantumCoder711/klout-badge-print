...
import { fetchPrinterCount } from '../store/slices/printerSlice';
...
const Card: React.FC<CardProps> = (props) => {
    ...
    const { printerCount, loading: printerLoading } = useAppSelector((state: RootState) => state.printer);
    ...
    const handleSelectTab = () => {
        dispatch(fetchPrinterCount(props.eventuuid));
    }
        ...
        <div className="max-w-80 shadow-xl rounded-lg relative">
            ...
            <div className='w-full flex gap-3 p-3'>
                ...
                <Dialog onOpenChange={(isOpen) => {
                    ...
                    {(printerCount !== null && printerCount !== 0) ? <DialogContent>
                        {!printerLoading && printerCount !== null ? (
                            <DialogHeader>
                                ...
                                {tab && (
                                    <div className='grid grid-cols-5 place-content-center gap-5'>
                                        {Array(printerCount).fill(0).map((_, index: number) => (
                                            <button 
                                                key={index + 1} 
                                                onClick={() => { 
                                                    setTab(false); 
                                                    setPrinterNumber(index + 1); 
                                                }} 
                                                className='p-5 grid place-content-center cursor-pointer bg-teal-500 font-bold text-2xl text-white rounded'
                                            >
                                                {index + 1}
                                            </button>
                                        ))}
                            ...
                            {!tab && !showShapes && (
                                <div className='flex gap-5 justify-center'>
                                    <button 
                                        ...
                                        className='bg-teal-500 grid rounded-md w-full h-40 text-white font-medium text-xl place-content-center'
                                    >
                                        Print Badge
                                    </button>
                                    <Link 
                                        to={`/print-name/${props.eventuuid}/${printerNumber}`} 
                                        className='bg-green-600 grid rounded-md w-full h-40 text-white font-medium text-xl place-content-center'
                                    >
                                        Print Name
                                    </Link>
                                </div>
                            ...
                            {showShapes && (
                                <div className='flex flex-wrap justify-center items-center gap-5 w-full'>
                                    <Link 
                                        to={`/print-badge/square/${props.eventuuid}/${printerNumber}`} 
                                        className='w-40 rounded-md h-40 grid place-content-center bg-green-600 text-white'
                                    >
                                        8 x 10 cm
                                    </Link>
                                    <Link 
                                        to={`/print-badge/rectangle/${props.eventuuid}/${printerNumber}`} 
                                        className='w-40 rounded-md h-60 grid place-content-center bg-red-600 text-white'
                                    >
                                        10.5 x 14.85 cm
                                    </Link>
                                    <Link 
                                        to={`/print-badge/combined/${props.eventuuid}/${printerNumber}`} 
                                        className='w-40 rounded-md h-60 grid place-content-center bg-blue-600 text-white'
                                    >
                                        4 x 6 inch
                                    </Link>
                                </div>
